...
'''区别divide文件夹：判断是否interface、exception'''
...
def findClassName(soup,classCount):
    ...
    tag_declare = soup.find('dib',attrs={'itemtype':'http://developers.google.com/ReferenceObject'})
    # if ("Exception" in className) or ("interface" in str(tag_declare)):
    if ("interface" in str(tag_declare)):#最后classesPage补了
        print("not class page")
        return "pass"

    addclassName = str(className) + "--" +str(classCount)#第classCount个url的类,从0计数
    os.mkdir("results" + "/" + addclassName )#每个类建立一个文件夹
...
def extractUrl(url,classCount,sheet1):
    ...
    if className=="pass":
        with open("not_class_url.txt",'a',encoding="utf-8") as f_not_class_url:
            f_not_class_url.write(str(classCount))
            f_not_class_url.write('\t')
            f_not_class_url.write(url)
            return False
...
def extractPermission(tag,className,methodCount,sheet1):
    ...
    path = "results" + "/" + className + "/"+methodName#在类名文件夹下创建函数名文件夹
    os.mkdir(path)
    #print  method
